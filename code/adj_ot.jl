using Distributed, SharedArrays, LinearAlgebra, Printf
...
function grad_ot_exp_parallel(received_data, c, rho, Nx, Ny, Nt, h, dt, source, source_position, receiver_position; pml_len=10, pml_coef=100, normalize_k=1, reg=1e-5, reg_m=1e2, OTiterMax=200);
    c = reshape(c, Nx, Ny)
    ...
    @sync @distributed for ind = 1:source_num
        ...
        # adjoint source
        data_forward_normalized = exp.(normalize_k * (data_forward))
        data_received_normalized = exp.(normalize_k * (received_data[:,:,ind]))
        dist, adj_source = adj_source_sinkhorn(data_forward_normalized, data_received_normalized, M; reg=reg, reg_m=reg_m, iterMax=OTiterMax, verbose=false);
...
function obj_fn_ot_parallel(received_data, c, rho, Nx, Ny, Nt, h, dt, source, source_position, receiver_position; pml_len=10, pml_coef=100, normalize_k=1, reg=1e-5, reg_m=1e2, OTiterMax=200);
    c = reshape(c, Nx, Ny)
    t = range(0,step=dt,length=Nt)
    M = cost_matrix_1d(t, t);
